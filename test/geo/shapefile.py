...
class ShapefileWriterestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.test_path = os.path.join(
            paths.disseration, "data", "minnabluff", "gis", "test_points.shp"
        )
        cls.points = [
            AgePoint(sample=a, y=b, x=c)
            for a, b, c in [
                ("MB07-186", -78.5323888889, 166.268833333),
    ...
    def test_something(self):
        ...
        writer.write_points(p, points=self.points, attrs=("sample", "material", "age", "age_error"))
