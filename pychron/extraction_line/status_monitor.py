...
class StatusMonitor(Loggable):
    ...
    def start(self, oid, vm):
        ...
        if not self._clients:
            ...
            self._thread = t
            self.debug(
                "Thread started: {} (daemon={}, alive={})".format(
            ...
            try:
                ...
                if self._thread is not None:
                    ...
                    if is_alive_before:
                        ...
                        self.debug(
                            "Attempting graceful thread shutdown with timeout={}s".format(
                ...
                self.debug(
                    ...
                    "clients={}, thread=None, stop_evt.is_set={}".format(
                        self._clients, self._stop_evt.is_set() if self._stop_evt else False
                    )
                )
                ...
                if block:
                    self.debug(
                        "Applying post-shutdown flush delay: {:.1f}s".format(
                ...
                self.warning(
                    ...
                    "type={}, error={}".format(type(shutdown_error).__name__, shutdown_error)
                )
