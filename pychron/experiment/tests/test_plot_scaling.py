...
class PlotScalingTestCase(unittest.TestCase):
    ...
    def test_large_signal_no_forced_zero_anchor(self):
        ...
        self.assertGreater(ymin, 390.0, "Large signal values should not be forced to ymin=0")
    ...
    def test_near_zero_small_variance_anchors_to_zero(self):
        ...
        self.assertEqual(ymin, 0, "Small values near zero should anchor to ymin=0 for clean plots")
    ...
    def test_medium_signal_no_anchor_to_zero(self):
        ...
        self.assertEqual(ymin, 0, "Boundary case: ymin == 50% of range should anchor to 0")
        ...
        self.assertGreater(ymin2, 0, "Values just above boundary should not anchor to 0")
    ...
    def test_negative_values_no_anchor_to_zero(self):
        ...
        self.assertLess(
            ymin, -5.0, "Negative values should have padding applied, not anchored to 0"
        )
    ...
    def test_padding_scales_with_data_range(self):
        ...
        ymin_small, ymax_small = self._calculate_y_limits(ys_small, "0.1", BASELINE)
        small_padding = ymax_small - max(ys_small)

        # Large range
        ys_large = [100.0, 200.0]  # Range: 100
        ymin_large, ymax_large = self._calculate_y_limits(ys_large, "0.1", BASELINE)
    ...
    def test_string_vs_float_ypadding(self):
        ...
        self.assertAlmostEqual(
            str_padding, 0.2, places=5, msg="String ypadding should be percentage of range"
        )
        self.assertAlmostEqual(
            float_padding, 0.1, places=5, msg="Float ypadding should be absolute value"
        )
    ...
    def test_all_limits_finite(self):
        ...
        for ys, ypad, kind in test_cases:
            ...
            self.assertTrue(isfinite(ymin), f"ymin must be finite for data {ys}")
            self.assertTrue(isfinite(ymax), f"ymax must be finite for data {ys}")
            self.assertLess(ymin, ymax, f"ymin must be less than ymax for data {ys}")
